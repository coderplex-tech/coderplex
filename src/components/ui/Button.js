...
export function Button({ children, variant = 'primary', size = 'md', to, onClick, className = "", disabled = false, type = 'button' }) {
    const buttonStyles = `${baseStyles} ${variants[variant]} ${sizes[size]} ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`;
    if (to && !disabled) {
        return (_jsx(Link, { to: to, className: buttonStyles, children: children }));
    }
    return (_jsx("button", { onClick: onClick, className: buttonStyles, disabled: disabled, type: type, children: children }));
}
